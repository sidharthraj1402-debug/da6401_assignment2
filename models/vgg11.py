...
class VGG11Encoder(nn.Module):
    ...
    def __init__(self, in_channels: int = 3, use_bn: bool = True):
        ...
        self.block1 = nn.Sequential(
            ...
            nn.BatchNorm2d(64) if use_bn else nn.Identity(), # BN normalizes activations; Identity() is a no-op when BN is disabled
...
# Justification: BN normalizes activations before ReLU, reducing internal covariate shift.
...
# Justification: FC layers hold ~120M parameters (25088×4096 + 4096×4096) and are the main source of overfitting. 
# Dropout forces redundant representations by training a different subnetwork each step.
# Conv layers are omitted because weight sharing limits overfitting, and spatial correlations make dropped values easy to infer from neighbors.
# p=0.5 balances regularization strength and signal retention.